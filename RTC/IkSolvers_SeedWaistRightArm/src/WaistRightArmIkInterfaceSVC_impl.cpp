// -*-C++-*-
/*!
 * @file  WaistRightArmIkInterfaceSVC_impl.cpp
 * @brief Service implementation code of WaistRightArmIkInterface.idl
 *
 */

#include "WaistRightArmIkInterfaceSVC_impl.h"
#include "WaistRightArmIkReturnID.h"

/*
 * Example implementational code for IDL interface RIGHT_IK_FAST::WaistRightArmIkInterface
 */
RIGHT_IK_FAST_WaistRightArmIkInterfaceSVC_impl::RIGHT_IK_FAST_WaistRightArmIkInterfaceSVC_impl()
{
 ...
 */
RIGHT_IK_FAST::RETURN_ID* RIGHT_IK_FAST_WaistRightArmIkInterfaceSVC_impl::solveRightArmIkSetWaistJoint(const RIGHT_IK_FAST::Rotation& eerot, const RIGHT_IK_FAST::FramePos& eetrans, const RIGHT_IK_FAST::WaistFrame& WaistPos, const RIGHT_IK_FAST::JointPos& nowJointPos, RIGHT_IK_FAST::JointPos_out solRightJointPos)
{
  std::cout << "solveRightArmIkSetWaistJoint" << std::endl;
  int ret; 
  double sol_eerot[9];
  ...
  solRightJointPos = new RIGHT_IK_FAST::JointPos;
  solRightJointPos->length(10);

  //================================================================================
  //FreeJoint0~(-135)で解を探す
    for(int j=0;j>-136;j--){
      ret = all_right_ik_solve(sol_eerot,sol_eetrans,sol,free_joint,&NumSolutions);
      if(!ret){
	//printf("IK計算完了0~-135\n");
  ...
  //FreeJoint0~135で解を探す
	...
	(*solRightJointPos)[end] = sol[LimitSolNum[MinAveNum]][end]*180/M_PI;
	/*
	if(end==4){
...
RIGHT_IK_FAST::RETURN_ID* RIGHT_IK_FAST_WaistRightArmIkInterfaceSVC_impl::solveRightArmIk(const RIGHT_IK_FAST::Rotation& eerot, const RIGHT_IK_FAST::FramePos& eetrans, const RIGHT_IK_FAST::JointPos& nowJointPos, RIGHT_IK_FAST::JointPos_out solRightJointPos)
{
  std::cout << "solveRightArmIk" << std::endl;

  RIGHT_IK_FAST::WaistFrame WaistPos;
  RIGHT_IK_FAST::RETURN_ID_var m_rid;
  WaistPos.yaw = 0;
  WaistPos.pitch = 0;
  WaistPos.roll = 0;
  //int i=0;
	  ...
	  for(int pitch_plus=0;pitch_plus<WaistPitch_LimitMax;pitch_plus++)
	    {
	      ...
	      WaistPos.roll = 0;
	      m_rid=solveRightArmIkSetWaistJoint(eerot,eetrans,WaistPos,nowJointPos,solRightJointPos);
  ...
  std::cout << "solveRightArmIk_IK計算できず" <<std::endl;
  RETURNID_NG;
  //RETURNID_OK;
}

RIGHT_IK_FAST::RETURN_ID* RIGHT_IK_FAST_WaistRightArmIkInterfaceSVC_impl::solveRightArmFk(const RIGHT_IK_FAST::JointPos& RightJointPos, RIGHT_IK_FAST::Rotation_out eerot, RIGHT_IK_FAST::FramePos_out eetrans)
{
  std::cout << "solveRightArmFk" << std::endl;
  eerot = new RIGHT_IK_FAST::Rotation;
  eerot->length(9);
  eetrans = new RIGHT_IK_FAST::FramePos;
  eetrans->length(3);
  double ikjoint[10];
  double calceerot[9];
  double calceetrans[3];
  ikjoint[0] = RightJointPos[0]*M_PI/180;
  ikjoint[1] = RightJointPos[1]*M_PI/180;
  ikjoint[2] = RightJointPos[2]*M_PI/180;
  ikjoint[3] = -RightJointPos[3]*M_PI/180;
  ikjoint[4] = -RightJointPos[4]*M_PI/180+M_PI/2;
  ikjoint[5] = -RightJointPos[5]*M_PI/180;
  ikjoint[6] = RightJointPos[6]*M_PI/180;
  ikjoint[7] = -RightJointPos[7]*M_PI/180;
  ikjoint[8] = -RightJointPos[8]*M_PI/180;
  ikjoint[9] = -RightJointPos[9]*M_PI/180; 
