...
int RightCartesianSpeed = 70;
int RightJointSpeed = 70;
...
JARA_ARM::RETURN_ID* JARA_ARM_ManipulatorCommonInterface_MiddleSVC_impl::moveLinearCartesianAbs(const JARA_ARM::CarPosWithElbow& carPoint)
{
  ...
  //CartesianSpeed判定
  if((RightCartesianSpeed <= 0)||(RightCartesianSpeed > 100)){
  ...
  int StrideNum = (int)(-3.3535*RightCartesianSpeed+336.35);//50*100/(CartesianSpeed);//CartesianSpeed
  double eerot[9];//姿勢
  ...
  int ret = noid.Solve_RightArmIk(eerot,eetrans,iksol);
  if(ret){
  ...
  //現在位置の読み取り
  noid.getRightJointAngle(JointPos);
  noid.Solve_RightArmFK(start_eerot,startPos,JointPos);
  ...
  for(int i=0; i<=StrideNum ; i++){
    ...
    */
    int ret = noid.Solve_RightArmIk(viaRot,viaPos,iksol);
    if(!ret){
  ...
  std::cout << "sumduration = " << duration <<std::endl;
  */
...
JARA_ARM::RETURN_ID* JARA_ARM_ManipulatorCommonInterface_MiddleSVC_impl::movePTPCartesianAbs(const JARA_ARM::CarPosWithElbow& carPoint)
{
  ...
  //JointSpeed判定
  if((RightJointSpeed <= 0)||(RightJointSpeed > 100)){
  ...
  //JointSpeedが70%で約3秒
  movetime = (int)(-100.71*RightJointSpeed + 10101);
